...
faceted_histogram <- ggplot(data = combined_data, aes(value)) +
  geom_histogram(colour = "black", fill = "white", bins = 30) +
  facet_grid(pollutant ~ scale, scales = "free_x") +
  labs(x = "Mean Values", y = "Frequency", title="Distribution of Mean Values",
...
CECILIA COME BACK TO THIS
```{r demographics.table}
acs_simplified %>% 
  summarise(White=White/Total_Pop*100,
            Black_AfricanAmerican=Black_AfricanAmerican/Total_Pop*100,
            AIAN=AIAN/Total_Pop,
            Asian=Asian/Total_Pop,
            Hawaiian_PacificIslander=Hawaiian_PacificIslander/Total_Pop,
            HispanicorLatino=HispanicorLatino/Total_Pop,
            Mean_PercentBelowPoverty=mean(Percent_Below_Poverty),
            SD_PercentBelowPovery=sd(Percent_Below_Poverty)
            )
...
```{r universal.kriging.prediction.no2.BLOCK, warning=FALSE, eval=FALSE}
### STILL NEED TO EDIT
...
# Estimate the variogram with a covariate predictor
v.uk.pm25 <- variogram(log_value ~ pop10_s05000 + lu_industrial_p03000 + log_m_to_a1 + log_m_to_a2 + log_m_to_a3 + log_m_to_coast + log_m_to_l_airp + log_m_to_comm, data=(annual_for_preds %>% filter(variable=="neph_bscat")))
                  #,cutoff=5000)
...
# sometimes has convergence issues 
m.uk.pm25 <- fit.variogram(v.uk, vgm(c("Exp", "Sph", "Mat")))
...
```{r universal.kriging.prediction.pm25.grid, warning=FALSE, eval=FALSE}
### STILL NEED TO EDIT
...
```{r universal.kriging.prediction.pm25.BLOCK, warning=FALSE, eval=FALSE}
### STILL NEED TO EDIT
