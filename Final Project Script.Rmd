...
uk_map <- (no2_uk_map | pm25_uk_map) + 
  plot_annotation(
    title = "Figure 3: Map of Study Area with UK NO2 and PM2.5 Predictions Overlaid as Points",
    theme = theme(
...
pm_full_results %>%
  ...
  kable(
    caption = "Table 5: Regression for Association Results for PM2.5 Model. Response variable was log(PM2.5), and all results have been exponentiated.",
    digits = 3, 
...
no2_full_results %>%
  ...
  kable(
    caption = "Table 6: Regression for Association Results for NO2 Model. Response variable was log(NO2), and all results have been exponentiated.",
    digits = 3, 
    col.names = c("Term", "Estimate", "Std. Error", "t-value", "P-value", "95% CI Low", "95% CI High")
  ) %>%
  kable_styling(
...
no2_english_map <- ggplot() +
  ...
  bi_scale_fill(pal = "PurpleGrn", dim = 3, name = NULL) +
  labs(
    title = "Figure 6: Relationship Between % English Language \nand Predicted NO2 Concentrations"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
