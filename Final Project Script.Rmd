...
# Chloropleth visualization of predicted pollutants with demographic variables

```{r pm25.chloropleth.map}
...
full_data_w_preds_perc_spatial <- st_as_sf(
  left_join(census_clean_sf, full_data_w_preds_perc_spatial, by = c("AFFGEOID" = "GEO_ID"))
) %>% 
  filter(!is.na(NAME.y))
...
combined_pm_chloropleth <- (pm_poverty_map + theme(legend.position = "none") |
...
combined_pm_chloropleth <- (combined_pm_chloropleth | wrap_elements(common_legend)) +
  plot_annotation(
    title = "Fiugre: Relationship between Predicted PM2.5 (ug/m^3)\nand Significant Demographic Variables from Regression Results",
    theme = theme(
...
print(combined_pm_chloropleth)
```


```{r no2.chloropleth.map}
## generating plots for significant predictors from regression results
