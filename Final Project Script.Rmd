...
# created stop data for use in regression for prediction
annual_for_preds <- annual_w_acs %>% select(c(location, variable,
                                          value, pop10_s05000,
                                          lu_industrial_p03000, log_m_to_a1, 
                                          log_m_to_a2, log_m_to_a3, log_m_to_coast,
                                          log_m_to_l_airp, log_m_to_comm, longitude, latitude))
...
percentbelowpoverty_stats <- acs_simplified %>%
  summarise(
    Mean_Percent_Below_Poverty = mean(Percent_Below_Poverty, na.rm = TRUE),
    Median_Percent_Below_Poverty = median(Percent_Below_Poverty, na.rm = TRUE),
    SD_Percent_Below_Poverty = sd(Percent_Below_Poverty, na.rm = TRUE)
  )
...
Delete below chunk? Does not make sense.

```{r}
#Visualization: Demographics, language, and poverty

# Where are these Count values from? What is this?
...
percentbelowpoverty_stats <- data.frame(
  Metric = c('Mean_Percent_Below_Poverty'),
...
TO-DO for the variograms -- add more of the code from Lab 6 to look at the cloud variograms and more.
```{r set.up.sf.data}
...
g + 
  ...
  labs(title = "Map of Study Area with the\nMobile Monitoring Data",
       col="NO2 (ppb)"
       )
...
vgm_no2_stop <- variogram(log_mean ~ 1, stop_for_preds %>% filter(variable == "no2"), cloud = TRUE)

# Variogram Cloud with annual_for_preds --> 32135
vgm_no2_annual <- variogram(value ~ 1, annual_for_preds %>% filter(variable == "no2"), cloud = TRUE)
...
plot(variogram(log_mean ~ 1, stop_for_preds %>% filter(variable == "no2")))

# Binned Variogram with annual data --> prettier!
plot(variogram(value ~ 1, annual_for_preds %>% filter(variable == "no2")))

# Binned Variogram with Point Counts
plot(variogram(value ~ 1, annual_for_preds %>% filter(variable == "no2")), pl = TRUE)
...
v_stop <- variogram(log_mean ~ 1, data=stop_for_preds %>% filter(variable == "no2"))
v_annual <- variogram(value ~ 1, data=annual_for_preds %>% filter(variable == "no2"))
# as before:
plot(v_stop)
plot(v_annual)
...
v_stop.uk <- variogram(log_mean ~ pop10_s05000 + lu_industrial_p03000, data=(stop_for_preds %>% filter(variable=="no2")),
                  cutoff=100000)

v_annual.uk <- variogram(value ~ pop10_s05000 + lu_industrial_p03000, data=(annual_for_preds %>% filter(variable=="no2")),
                  cutoff=100000)
plot(v_stop.uk)
plot(v_annual.uk)
...
```{r universal.kriging.prediction.no2.grid, warning=FALSE}
#transform stop data to Lambert for distances 
...
# Fit the universal kriging model for NO2 stop data and predict on the grid --> doesn't run
# ln.no2.kr <- krige(log_mean ~ log_m_to_a1 + log_m_to_a2 + log_m_to_a3 + log_m_to_coast + log_m_to_l_airp + log_m_to_comm + pop10_s05000 + lu_industrial_p03000, 
#                  stop_for_preds %>% filter(variable=="no2"), # CRS in meters
#                  newdata = grid_for_preds, # CRS in meters
#                  model = m_stop.uk)

# Fit the universal kriging model for NO2 annual data and predict on the grid 
ln.no2.kr <- krige(value ~ log_m_to_a1 + log_m_to_a2 + log_m_to_a3 + log_m_to_coast + log_m_to_l_airp + log_m_to_comm + pop10_s05000 + lu_industrial_p03000, 
...
# Plot UK predictions --> nothing generating on the map
pl3 <- plot(ln.no2.kr["var1.pred"], main = "UK Prediction of Log(NO2)")

# Plot UK prediction standard errors --> nothing generating on the map
...
ln.PM25.kr <- krige(log_mean ~ log_m_to_a1 + log_m_to_a2 + log_m_to_a3 + log_m_to_coast + log_m_to_l_airp + log_m_to_comm + pop10_s05000 + lu_industrial_p03000, 
                 stop_for_preds %>% filter(variable=="neph_bscat"), # CRS in meters
                 newdata = grid_for_preds, # CRS in meters
                 model = m.uk)
