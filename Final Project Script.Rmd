...
stop_clean %>% 
  ...
  kable(caption = "Table 1: Descriptive statistics for NO2 and PM2.5 for Study Area", digits = 2) %>% 
  kable_styling()
...
annual_clean %>% 
  ...
  rename(Pollutant = variable) %>% 
  kable(caption = "Table 2: Descriptive statistics for Annual NO2 and PM2.5 Measurements Across Study Area Monitoring Locations (N = 304)", digits = 2) %>% 
...
**Table 3** describes the demographic data for the study area by county. 
...
table1 %>% 
  kable(
    caption = "Table 2: Demographics by County for All Census Tracts included in study. All demographics are presented as percents besides total population and number of census tracts.",
    digits = 2
  ) %>%
  kable_styling()
...
Below still needs to be fixed (full one is fixed though).
```{r}
...
```{r q3.univariate.reg.assoc.no2, eval=FALSE}
# Regression between annual PM2.5 concentrations and various demographics
...
## Edited - cm
#PM2.5 x poverty
pm_poverty <- lm(`ln(NO2) ln(ppb)` ~ Percent_Below_Poverty, 
                 data = full_data_w_preds)
pm_poverty_res <- tidy(pm_poverty, conf.int=TRUE, exponentiate=TRUE) %>%
...
# PM2.5 x White
pm_white <- lm(`ln(NO2) ln(ppb)` ~ p_White, data = full_data_w_preds_perc)
pm_white_res <- tidy(pm_white, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_White" ~ 4.40,
                             .default=NA))

# PM2.5 x Black African American
pm_BlackAA <- lm(`ln(NO2) ln(ppb)` ~ p_BlackAA, data = full_data_w_preds_perc)
pm_BlackAA_res <- tidy(pm_BlackAA, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_BlackAA" ~ 4.31,
                             .default=NA))

# PM2.5 x AIAN
pm_AIAN <- lm(`ln(NO2) ln(ppb)` ~ p_AIAN, data = full_data_w_preds_perc)
pm_AIAN_res <- tidy(pm_AIAN, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_AIAN" ~ 4.40,
                             .default=NA))

# PM2.5 x Asian
pm_Asian <- lm(`ln(NO2) ln(ppb)` ~ p_Asian, data = full_data_w_preds_perc)
pm_Asian_res <- tidy(pm_Asian, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_Asian" ~ 4.39,
                             .default=NA))

# PM2.5 x HPI
pm_HPI <- lm(`ln(NO2) ln(ppb)` ~ p_HPI, data = full_data_w_preds_perc)
pm_HPI_res <- tidy(pm_HPI, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_HPI" ~ 4.38,
                             .default=NA))

# PM2.5 x Hispanic
pm_Hispanic <- lm(`ln(NO2) ln(ppb)` ~ p_Hispanic, data = full_data_w_preds_perc)
pm_Hispanic_res <- tidy(pm_Hispanic, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_Hispanic" ~ 4.36,
                             .default=NA))

# PM2.5 x language
pm_engl_only <- lm(`ln(NO2) ln(ppb)` ~ p_engl_only, data = full_data_w_preds_perc)
pm_engl_only_res <- tidy(pm_engl_only, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_engl_only" ~ 4.38,
                             .default=NA))

pm_lang_other <- lm(`ln(NO2) ln(ppb)` ~ p_lang_other, data = full_data_w_preds_perc)
pm_lang_other_res <- tidy(pm_lang_other, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_lang_other" ~ 4.38,
                             .default=NA))

pm_spanish <- lm(`ln(NO2) ln(ppb)` ~ p_spanish, data = full_data_w_preds_perc)
pm_spanish_res <- tidy(pm_spanish, conf.int=TRUE, exponentiate=TRUE) %>%
  mutate(intercept=case_when(term=="p_spanish" ~ 4.36,
                             .default=NA))
...
pm_univar_res %>%
  ...
  kable(
    caption = "Table: Univariate Regression for Association Results for PM2.5 Model. Response variable was log(NO2), and all results have been exponentiated. Each row represents a separate univariate regression model, with the Term as the predictor variable.",
    digits = 3, 
    col.names = c("Term", "Intercept", "Estimate", "Std. Error", "t-value", "P-value", "95% CI Low", "95% CI High")
  ) %>%
  kable_styling(
...
### Variograms for NO2 UK Predictions
...
### UK Predictions of log(NO2) on Grid and Census Block Centroids
...
# Plot UK prediction standard errors
pl4 <- plot(ln.NO2.block.kr["se"], main = "UK Prediction Error PM2.5 (Standard Error) on Census Block Centroids")
```

## PM2.5 Variograms and UK Predictions

### Variograms for PM2.5 UK Predictions
...
### UK Predictions of log(PM2.5) on Grid and Census Block Centroids
```{r UK.preds.logPM25}
...
pl4 <- plot(ln.PM25.kr["se"], main = "UK Prediction Error PM2.5 (Standard Error) on Census Block Centroids")
```
